...
导入 skill 后，运行以下命令注册 `/hzero-i18n-*` 命令到 opencode：
...
注册后重启 opencode 即可使用 `/hzero-i18n-*` 命令。

## Quick Reference

| 命令 | 说明 | 关键参数 |
|------|------|----------|
| `/hzero-i18n-query` | 查询多语言 | promptKey, promptCode, description |
| `/hzero-i18n-add` | 新增多语言 | promptKey, promptCode, promptConfigs |
| `/hzero-i18n-modify` | 修改多语言 | promptId, objectVersionNumber, _token, promptConfigs |
| `/hzero-i18n-delete` | 删除多语言 | promptId, objectVersionNumber, _token |
| `/hzero-i18n-check` | 检查代码多语言问题 | 文件/目录路径 |
| `/hzero-i18n-translate` | AI 翻译未翻译条目 | promptKey, 目标语言 |
| `/hzero-i18n-export` | 生成 Excel/CSV | promptKey, 格式(xlsx/csv) |
| `/hzero-i18n-import` | 从文件导入 | 文件路径 |
| `/hzero-i18n-update` | 检查 skill 版本更新 | 分支(可选,默认master) |
| `/hzero-i18n-release` | 发布新版本（变更→changelog→审批→提交→push） | 版本号(可选) |

## Environment Setup

首次使用时检查 `hzero-i18n/.env.json`，如果 `projects` 为空则引导用户配置：
...
`update` 为可选字段（用户手动配置），用于版本更新检查（`/hzero-i18n-update`）的 HTTP/HTTPS 代理。执行更新检查前，若该字段存在，AI 优先将其设为 `HTTP_PROXY`/`HTTPS_PROXY` 环境变量再运行（axios 自动读取）；未配置则沿用系统已有的代理环境变量。
...
> 手动执行 `/hzero-i18n-update` 命令为**强制检查**（`node scripts/update.js`，始终联网，不读 cache 判断是否跳过），但检查后写入 `lastCheckDate`；本节 cache-first（读 cache 判断是否跳过）仅适用于每日自动检查。
