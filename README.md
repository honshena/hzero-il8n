...
```powershell
# opencode
Copy-Item "<skill目录>\commands\hzero-front-i18n-*.md" "$env:USERPROFILE\.config\opencode\commands\" -Force
# Claude Code
Copy-Item "<skill目录>\commands\hzero-front-i18n-*.md" "$env:USERPROFILE\.claude\commands\" -Force
```

**Codex**：不支持 `/hzero-front-i18n-*` 斜杠命令，直接用自然语言触发，例如："用 hzero-front-i18n 查询 hskp.common"。
...
Token 必须有 0 租户平台层权限（`getPromptByLang` 为公开接口，纯查询/检查可留空,其他接口均需要使用token）。`fileProjectMap` 记录文件路径与项目的关联，由 AI 自动维护，详见 SKILL.md。
...
| 命令 | 说明 | 关键参数 |
|------|------|----------|
| `/hzero-front-i18n-query` | 查询多语言 | promptKey, promptCode |
| `/hzero-front-i18n-add` | 新增多语言 | promptKey, promptCode, promptConfigs |
| `/hzero-front-i18n-modify` | 修改多语言 | promptId, objectVersionNumber, _token |
| `/hzero-front-i18n-delete` | 删除多语言 | promptId（逐条审批） |
| `/hzero-front-i18n-check` | 检查代码多语言问题（含英文大小写） | 文件/目录路径 |
| `/hzero-front-i18n-translate` | AI 翻译未翻译条目 | promptKey, 目标语言 |
| `/hzero-front-i18n-export` | 导出 Excel/CSV | promptKey, 格式 |
| `/hzero-front-i18n-import` | 从 Excel/CSV 导入 | 文件路径 |
...
├── README.md                # 本文档
├── commands/                # /hzero-front-i18n-* 斜杠命令定义
