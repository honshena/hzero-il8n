...
**检查开始前**，用 `question` 工具（`multiple: true`）列出检查项（首个为「全部」，其余为硬编码字符串、未注册 key、翻译缺失、code 格式、英文大小写规范、intl.get 作用域）让用户多选要执行哪些；选「全部」则执行所有，否则仅对选中项检查。完成后将问题写入 data.json 并逐条列表展示（含位置/类型/当前值/建议值）。
